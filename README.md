...
To mitigate against false verification results (`matches_onchain` being incorrect), program upgrades are listened for via a Helius webhook to keep `on_chain_hash` in sync. All programs are also fetched every ~5 minutes in case a webhook is missed or a program is closed.
